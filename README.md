...
WiSSkHy is a **local** and **flexible** environment built with R-Shiny and SQLite for hydrological data management based on an SQLite database and fully open-source tools. WiSSkHy is a local environment built with R-Shiny and SQLite for hydrological data management, relying on an SQLite database and fully open-source tools. Its database architecture follows the [OGC SensorThings API standard](https://docs.ogc.org/is/18-088/18-088.html) (ISO 19156-based).
